...
init({_Any, http}, Req=#http_req{socket=Socket}, []) ->
	...
	{ok , Req3}= cowboy_http_req:chunk( [ "queue=", Queue , ", receving ....."  , "\r\n"] , Req2 ),
	  ...
	  lists:foreach( fun(Event) -> 
       ...
       Sub = #'basic.consume'{queue = Queue ,no_ack= false},
	   ...
	   {loop , Req3,  #state{app=App,events=binary:split(Events , <<",">>), channel=Channel, 
...
info(  {#'basic.deliver'{delivery_tag = _Dtag}, #amqp_msg{payload=Payload}}   , 
	   Req= #http_req{socket=Socket}  ,  State =#state{channel=_Channel} )  ->
	cowboy_tcp_transport:setopts(Socket, [{active, once}]), 
	{ok,Req2} = cowboy_http_req:chunk( [Payload, "\r\n"] , Req ),
